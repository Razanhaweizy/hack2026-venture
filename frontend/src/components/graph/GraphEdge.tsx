/**
 * GraphEdge Component
 * Renders an edge between two nodes with confidence-based coloring
 */
...
import { 
  EDGE_THICKNESS, 
  EDGE_OPACITY, 
  getConfidenceEdgeColor, 
  getConfidenceEdgeOpacity 
} from './constants';
...
interface GraphEdgeProps {
  ...
  targetConfidence?: number; // Confidence of the child/target node
}
...
export function GraphEdge({
  ...
  targetPos,
  isHighlighted,
  targetConfidence = 50, // Default to medium confidence
}: GraphEdgeProps) {
  // Get visual properties based on edge type and target confidence
  const color = getConfidenceEdgeColor(targetConfidence);
  const thickness = EDGE_THICKNESS[edge.type];
  const baseOpacity = getConfidenceEdgeOpacity(targetConfidence, EDGE_OPACITY[edge.type]);
