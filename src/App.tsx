...
import { ClaimCriticPanel } from './components/critic';
import { critiqueClaim } from './core/services';
import type { ClaimCriticResult, ClaimCriticError } from './core/services';
...
interface SidePanelProps {
  ...
  onCritiqueClaim: () => void;
  criticLoading: boolean;
  showCriticPanel: boolean;
  criticResult: ClaimCriticResult | ClaimCriticError | null;
  onCloseCriticPanel: () => void;
  onDeleteClaim: () => void;
}

function SidePanel({ selectedNode, progress, onAddTestClaim, onAddSpecificClaim, onTestPipeline, onOpenDiffPanel, onTestDiffSystem, onOpenFollowUp, onTestVagueFollowUp, onShowNodeHistory, onCreateSnapshot, onTestVersionHistory, onExportCSV, pipelineResult, isDiffOpen, isFollowUpOpen, hasFollowUpData, isTimeTraveling, onCritiqueClaim, criticLoading, showCriticPanel, criticResult, onCloseCriticPanel, onDeleteClaim }: SidePanelProps) {
  return (
    <div
      style={{
        position: 'absolute',
    ...
    >
      {showCriticPanel && selectedNode?.type === 'claim' && (
        <ClaimCriticPanel
          claimTitle={selectedNode.title}
          result={criticResult}
          loading={criticLoading}
          onClose={onCloseCriticPanel}
        />
      )}
      {/* Header */}
      <h1 style={{ 
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        {selectedNode ? (
          <div style={{
            ...
            {/* Critique claim (AI agent) - only for claim nodes */}
            {selectedNode.type === 'claim' && (
              <>
                <button
                  onClick={onCritiqueClaim}
                  disabled={criticLoading}
                  style={{
                    marginTop: '8px',
                    width: '100%',
                    padding: '8px 12px',
                    background: criticLoading ? '#E5E7EB' : COLORS.sequoiaGreen,
                    color: criticLoading ? '#9CA3AF' : 'white',
                    border: 'none',
                    borderRadius: '6px',
                    fontSize: '12px',
                    fontWeight: 500,
                    cursor: criticLoading ? 'not-allowed' : 'pointer',
                  }}
                >
                  {criticLoading ? 'Critiquing...' : 'Critique claim (AI)'}
                </button>
                <button
...
function App() {
  ...
  const [criticResult, setCriticResult] = useState<ClaimCriticResult | ClaimCriticError | null>(null);
  ...
  const handleCloseCriticPanel = useCallback(() => {
    setShowCriticPanel(false);
  ...
  const handleCritiqueClaim = useCallback(async () => {
    if (!selectedNode || selectedNode.type !== 'claim') return;
    const apiKey = import.meta.env.VITE_GEMINI_API_KEY as string | undefined;
    if (!apiKey?.trim()) {
      setCriticResult({ message: 'Gemini API key not set. Add VITE_GEMINI_API_KEY to .env.', code: 'NO_API_KEY' });
    ...
    try {
      const store = useGraphStore.getState();
      const parents = store.getParents(selectedNode.id);
      const frameworkParent = parents.find(p => p.node.type === 'framework');
      const frameworkContextTitle = frameworkParent ? frameworkParent.node.title : undefined;
      const result = await critiqueClaim(
        {
          title: selectedNode.title,
          content: selectedNode.content,
          frameworkContextTitle,
        },
        { apiKey }
      );
  ...
  return (
    <div style={{ 
      ...
      <SidePanel
        ...
        onCritiqueClaim={handleCritiqueClaim}
        criticLoading={criticLoading}
        showCriticPanel={showCriticPanel}
        criticResult={criticResult}
        onCloseCriticPanel={handleCloseCriticPanel}
